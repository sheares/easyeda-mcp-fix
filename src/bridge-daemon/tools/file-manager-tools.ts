...
async function fetchCurrentSourceAndContext(
	...
	const result = await ctx.sendToExtension('fileManager.getDocumentSource', params) as {
		source: string;
...
export function fileManagerTools(ctx: ToolContext): ToolDef[] {
	return [
		{
...
for a structured validation report.${WORKFLOW_HINT}`,
			...
			handler: async ({ instance_id, ...rest }) => {
				const result = await ctx.sendToExtension('fileManager.getDocumentSource', { instance_id, ...rest }) as {
					source: string;
					context?: DocumentContext;
				};
				...
				validateByDocType(result.source, docCtx.documentType, {
					projectUuid: docCtx.projectUuid,
					documentUuid: docCtx.documentUuid,
...
that the data is bad.${WORKFLOW_HINT}`,
			...
			handler: async ({ filePath, instance_id, ...rest }) => {
				const result = await ctx.sendToExtension('fileManager.getDocumentSource', { instance_id, ...rest }) as {
					source: string;
					context?: DocumentContext;
				};
				...
				const docCtx = result.context ?? {};
				const validation = await validateByDocType(result.source, docCtx.documentType, {
					projectUuid: docCtx.projectUuid,
					documentUuid: docCtx.documentUuid,
				});
...
other types skip with a status.${WORKFLOW_HINT}`,
			...
			handler: async ({ filePath, instance_id, document, validate }) => {
				const mode = validate ?? UPLOAD_VALIDATE_DEFAULT;
...
All internal files are human-readable newline-delimited JSON arrays.${WORKFLOW_HINT}`,
			...
			handler: async ({ filePath, instance_id, ...rest }) => {
				const result = await ctx.sendToExtension('fileManager.getProjectFile', { instance_id, ...rest }) as { fileName: string; data: string; size: number };
...
returned backup.sha references the pre-import state.${WORKFLOW_HINT}`,
			...
			handler: async ({ filePath, instance_id, existingProjectUuid, ...rest }) => {
				...
				if (existingProjectUuid) {
					backup = await backupProject(ctx, {
						instance_id,
				...
				const result = await ctx.sendToExtension('fileManager.importProjectByProjectFile', {
