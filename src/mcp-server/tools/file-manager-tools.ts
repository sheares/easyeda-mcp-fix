...
use to find the prior state if the edit goes wrong.`;
...
Returns { success, backup: { sha, path } } on success, or throws if the format is invalid
or if the pre-edit backup could not be written.
...
and committed to a local git-tracked repo — the returned backup.sha references the
pre-edit state.${WORKFLOW_HINT}`,
		withDocumentParam({
			source: z.string().describe('The complete document source code to set'),
		}),
		async ({ source, instance_id, document }) => {
...
before the replacement and committed to a local git-tracked repo — the returned backup.sha
references the pre-edit state.${WORKFLOW_HINT}`,
		withDocumentParam({
		...
		async ({ filePath, instance_id, document }) => {
			...
			const result = await bridge.send('fileManager.setDocumentSource', { source, instance_id, document }) as Record<string, unknown>;
			return { content: [{ type: 'text', text: JSON.stringify({ ...result, loaded: filePath, size: source.length, backup, note: formatBackupSummary(backup) }, null, 2) }] };
		},
	);
